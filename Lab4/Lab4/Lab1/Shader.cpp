...
#include <fstream>
#include <vector>
Shader::Shader(const char* vertexSource, const char* fragmentSource, const char* geometrySoure = NULL)
{
	program = glCreateProgram(); // create shader program (openGL saves as ref number)
	
	shaders.push_back( CreateShader(LoadShader(vertexSource), GL_VERTEX_SHADER)); // create vertex shader
	shaders.push_back(CreateShader(LoadShader(fragmentSource), GL_FRAGMENT_SHADER)); // create fragment shader
	if (geometrySoure != NULL) {
		shaders.push_back(CreateShader(LoadShader(geometrySoure), GL_GEOMETRY_SHADER));
	}
...
Shader::~Shader()
{
	...
	glDetachShader(program, shad); //detach shader from program
	glDeleteShader(shad); //delete the shaders
}
